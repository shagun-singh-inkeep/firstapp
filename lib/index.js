...
📚 **Sources:**
${commentLinks.links.map((link, index) => `${index + 1}. [${link.title || link.url}](${link.url})`).join('\n')}`;
        }
        ...
        if (!shouldRespondComment) {
            ...
            await (0, logToInkeepAnalytics_1.logToInkeepAnalytics)({
                ...
                userProperties: userProps,
                aiProvidedLinks: (commentLinks?.links || []).map((l) => ({
                    url: l.url,
                    title: l.title ?? undefined
                })),
            });
