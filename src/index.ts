...
app.on("issue_comment.created", async (context: Context<"issue_comment.created">) => {
  ...
  try {
    ...
    const fullThreadMessages: Messages[] = [
    ...
    const issueProps = { 
      ...
      threadContext: true,
      totalThreadMessages: fullThreadMessages.length,
    ...
    const { text, toolCalls } = await generateText({
      ...
      messages: [
        ...
        // The new mention comment as the final user message
        { id: 'new-mention', role: 'user', content: commentBody }
      ],
      tools: {
        provideAIAnnotations: { parameters: ProvideAIAnnotationsToolSchema },
        provideLinks: { parameters: ProvideLinksToolSchema }
    ...
    const messagesToLogToAnalytics: Messages[] = [
      ...fullThreadMessages, // Full thread context
      { role: 'assistant', content: text } // Bot's response
    ];
