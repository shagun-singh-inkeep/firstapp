 ...
 */
export function queryQueryPropertyValues(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      components.PropertyValuesRequestBody$outboundSchema.optional().parse(
        value,
      ),
  ...
  const path = pathToFunc("/query/property-values")();
  ...
  const context = {
    ...
    operationID: "queryPropertyValues",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.PropertyValuesResponseBody$inboundSchema),
